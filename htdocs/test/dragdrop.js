...
// $project_id = $_GET['p'];
// ?>
urlquery=location.href.split("?")rnurlterms=urlquery[1].split(",");
var projectId = urlquery[0];
...
jQuery(document).ready(function() {
	...
	playFile('http://wikistudios.caryme.com/playlist.php?p='+projectId);
	
    $("#clip-list").sortable({
      handle: '.thumbnail',
      update : function () {
			//var project = "http://wikistudios.caryme.com/playlist.php?p=<?=$project_id?>";
			var project = "http://wikistudios.caryme.com/playlist.php?p="+projectId;
			...
			$.get("video-sort.php?p="+projectId+"&"+order);
			// videoPlayer.sendEvent('LOAD', 'http://wikistudios.caryme.com/playlist.php?p=<?$project_id?>');
			videoPlayer.sendEvent('LOAD', 'http://wikistudios.caryme.com/playlist.php?p='+projectId);
